    ...
    <nav className="navbar">
      <div className="navbar__inner">
        ...
        <Link to={ROUTES.HOME} className="navbar__logo">  
          <img src="/logo/logo1.PNG" alt="Hanni Logo" className="logo-img" />Hanni
        </Link>

        {/* Desktop nav links */}
        <ul className="navbar__links">
          <li><NavLink to={ROUTES.HOME} end className={({ isActive }) => isActive ? 'active' : ''}>HOME</NavLink></li>
          <li><NavLink to={ROUTES.PRODUCTS} className={({ isActive }) => isActive ? 'active' : ''}>PRODUCTS</NavLink></li>
          {user?.role === 'SELLER' && (
            <li><NavLink to={ROUTES.ADMIN.DASHBOARD} className={({ isActive }) => isActive ? 'active' : ''}>MY SHOP</NavLink></li>
          )}
        </ul>
          ...
          {isAuthenticated ? (
            <div className="navbar__user">
              <button className="navbar__user-btn" onClick={() => setMenuOpen(!menuOpen)}>
                <span className="user-avatar">{user?.first_name?.charAt(0) ?? '?'}</span>
                <span className="user-name">{user?.first_name || user?.username}</span>
                <span className="chevron">{menuOpen ? '▲' : '▼'}</span>
              </button>
              {menuOpen && (
                <div className="navbar__dropdown">
                  <Link to={ROUTES.ORDERS} onClick={() => setMenuOpen(false)}>Order History</Link>
                  <Link to={ROUTES.PROFILE} onClick={() => setMenuOpen(false)}>Profile</Link>
